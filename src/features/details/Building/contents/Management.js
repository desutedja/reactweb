...
function Component({ view }) {
  ...
  const { selected, management, loading, refreshToggle } = useSelector(
  ...
  const columnsManagement = [
    { Header: "ID", accessor: "id" },
    {
      ...
      accessor: (row) => (
        <a
          class="Link"
          href={"/" + auth.role + "/management/" + row.management_id}
        >
          {row.management_name}
        </a>
      ),
    ...
    {
      Header: "Billing Published",
      accessor: (row) => "Day " + row.billing_published,
    },
    {
      Header: "Billing Duedate",
      accessor: (row) => "Day " + row.billing_duedate,
    },
    {
      Header: "Billing Penalty Fee",
      accessor: (row) => row.penalty_fee + " %",
    },
    { Header: "Courier Fee", accessor: (row) => toMoney(row.courier_fee) },
    {
      Header: "Internal Courier Markup",
      accessor: (row) => row.courier_internal_markup + " %",
    },
    {
      Header: "External Courier Markup",
      accessor: (row) => row.courier_external_markup + " %",
    },
    {
      Header: "Status",
      accessor: (row) => {
  ...
  return (
    <>
      ...
      >
        ...
        >
          Are you sure to set <strong>{managementChose.management_name}</strong>{" "}
          as
          {managementChose.status === "active" ? " Active" : " Inactive"}?
        </h4>
        {managementChose.status === "active" && (
          <span
            style={{
              color: "#ff1212",
              fontSize: ".97rem",
          ...
          >
            This will make all other management Inactive.
          </span>
        )}
      </Modal>
      <Modal
      ...
      >
        ...
        >
          ...
          <Input
            label="In Range"
            type="number"
            inputValue={selectedRow.in_range}
            addons="km"
          />
          <Input
            ...
            type="select"
            options={banks}
            inputValue={selectedRow.settlement_bank}
          />
          <Input
            label="Settlement Account No"
            inputValue={selectedRow.settlement_account_no}
          />
          <Input
          ...
          <Input
            ...
            type="radio"
            name="auto_assign"
            inputValue={selectedRow.auto_assign}
            options={[
          ...
          <Input
            ...
            type="number"
            addons="task(s)"
            inputValue={selectedRow.auto_assign_limit}
          />
          <Input
            label="Auto Assign Schedule"
            type="radio"
            name="auto_assign_schedule"
            inputValue={selectedRow.auto_assign_schedule}
            options={[
          ...
          <Input
            ...
            type="number"
            addons="day(s)"
            inputValue={selectedRow.auto_assign_schedule_day}
          />
          <Input
            label="Courier Fee"
            type="number"
      ...
      <Table
        ...
        fetchData={useCallback(
          (pageIndex, pageSize, search) => {
            dispatch(
              getBuildingManagement(pageIndex, pageSize, search, selected)
            );
            // eslint-disable-next-line react-hooks/exhaustive-deps
          },
