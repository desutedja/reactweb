...
function Component({ view }) {
  ...
  const { disbursement, refreshToggle, loading } = useSelector(
    (state) => state.billing
  );
  ...
  const columns = useMemo(
    () => [
      {
        Header: "Billing Refcode",
        accessor: (row) => (
          <Link
            href="#"
            className="Link"
            to={"/" + role + "/billing/disbursement/" + row.trx_code}
          >
            {row.trx_code}
          </Link>
        ),
      ...
      { Header: "Amount", accessor: (row) => toMoney(row.selling_price) },
      {
        ...
        accessor: (row) =>
          row.disbursement_date ? (
            <Pill color="success">Disbursed</Pill>
          ) : (
      ...
      },
      {
        ...
        accessor: (row) => toSentenceCase(row.payment_bank),
      },
  ...
  return (
    <div>
      ...
      <h2 className="PageTitle">Billing Disbursement</h2>
      <Modal
        isOpen={modal}
        toggle={() => {
          // setSelected([]);
        ...
        disabledOk={transferCode.length === 0 || selected.length === 0}
        onClick={() => {
          ...
          dispatch(
            post(
              endpointBilling + "/management/billing/disbursement/flag",
              dataDisbursement,
              (res) => {
                dispatch(refresh());
      ...
      <div className="Container">
        ...
        >
          ...
          >
            ...
            <AnimatedNumber
              className="BigNumber"
              value={info.total_undisburse_amount}
        ...
        >
          ...
          >
            ...
            <AnimatedNumber
              className="BigNumber"
              value={info.total_disburse_amount}
              formatValue={formatValue}
            />
          </div>
        </div>
      ...
      <div className="row no-gutters">
        <div className="col-12 col-md-5 col-lg-3">
          <Card className="Container" style={{ boxShadow: "none" }}>
              ...
              <div className="col-auto">
                ...
                >
                  ...
                  {filtersDisbursement.map((item) => (
                    <option selected={item.value === filter} value={item.value}>
                      {item.label}
                    </option>
                  ))}
                </select>
            ...
            >
              {!loading && disbursement.items.length > 0
                ? disbursement.items.map((el, index) => (
                    <ListGroupItem
                      key={index}
                      onClick={() => {
                        if (
                          !selectedManagement.some((item) => item.id === el.id)
                        ) {
                          setSelectedManagement([...selectedManagement, el]);
                          return;
                        }
                        setSelectedManagement(
                          selectedManagement.filter((item) => item.id !== el.id)
                        );
                      }}
                      active={selectedManagement.some(
                        (item) => item.id === el.id
                      )}
                      action
                      tag="a"
                    ...
                    >
                      <div style={{ display: "block" }}>
                        ...
                        {(!el.settlement_account_name ||
                          !el.settlement_account_no ||
                          !el.settlement_bank) && (
                          <div style={{ color: "red" }}>
                            <FiAlertCircle /> Disbursement account information
                            not set
                          </div>
                        )}
                      </div>
                ...
                : !loading && (
                    <div className="w-100 text-center">No Management found</div>
                  )}
            </ListGroup>
          </Card>
        </div>
              ...
              >
                ...
                {view ? null : (
                  <MyButton
                    ...
                    onClick={() => {
                      setSelected(
                        data.filter((el) => el && !el.disbursement_date)
                      );
                      setModal(true);
                    }}
                  />
                ...
                <Button
                  ...
                  onClick={() =>
                    dispatch(
                      getFile(
                        endpointBilling +
                          "/management/billing/disbursement/list/transaction" +
                          "?management_id=" +
                          selectedManagement.map((item) => item.id).join(",") +
                          "&disbursement_date_min=" +
                          (status === "disbursed" ? disbursedStart : "") +
                          "&disbursement_date_max=" +
                          (status === "disbursed" ? disbursedEnd : "") +
                          "&settlement_date_min=" +
                          settledStart +
                          "&settlement_date_max=" +
                          settledEnd +
            ...
            <Card className="Container" style={{ boxShadow: "none" }}>
              <Table
                onSelection={(selectedRows) => {
                  setSelected(
                    selectedRows.filter((el) => el && !el.disbursement_date)
                  );
                }}
                filterExpanded={true}
                noContainer={true}
                ...
                fetchData={useCallback(
                  (pageIndex, pageSize, search) => {
                    ...
                    dispatch(
                      get(
                        endpointBilling +
                          "/management/billing/disbursement/list/transaction?limit=" +
                          pageSize +
                          "&page=" +
                          (pageIndex + 1) +
                          "&search=" +
                    ...
                    dispatch(
                      get(
                        endpointBilling +
                          "/management/billing/disbursement/management/amount" +
                          "?management_id=" +
                          selectedManagement.map((item) => item.id).join(",") +
                          "&payment_channel=" +
                          paymentChannel +
                ...
                filters={[
                  {
                    ...
                    delete: () => {
                      setSettledStart(monthStart());
                      setSettledEnd(monthEnd());
                    },
                    value:
                    ...
                    ? [
                        {
                          ...
                          delete: () => {
                            setDisbursedStart(today);
                            setDisbursedEnd(today);
                          },
                          value: isRangeToday(disbursedStart, disbursedEnd)
                            ? "Today"
                  ...
                  },
                  {
                    ...
                    value: paymentChannel
                      ? toSentenceCase2(paymentChannel)
                      : "All",
                    component: (toggleModal) => (
                      <Filter
