...
import {
  ...
  refresh,
  downloadTransactionDisbursement,
...
function Component({ view }) {
  ...
  const startOfMonth = moment().startOf("month").format("yyyy-MM-DD", "day");
  const endOfMonth = moment().endOf("month").format("yyyy-MM-DD", "day");
  const [settledStart, setSettledStart] = useState(startOfMonth);
  const [settledEnd, setSettledEnd] = useState(endOfMonth);
  const [disbursedStart, setDisbursedStart] = useState(startOfMonth);
  const [disbursedEnd, setDisbursedEnd] = useState(endOfMonth);

  const { loading, refreshToggle, disbursement } = useSelector(
  ...
  const getSum = (items) => {
    return items.reduce((sum, el) => {
      return type === "merchant"
        ? sum + el.total_selling_price + el.service_additional_price
        : sum + el.assignee_fee;
    }, 0);
  };
    ...
    if (type === "merchant")
      return [
        ...
        },
        {
          ...
          accessor: (row) =>
            type === "merchant"
              ? toMoney(row.total_selling_price + row.service_additional_price)
              : toMoney(row.assignee_fee),
        },
        {
          Header: "Disbursement Status",
          accessor: (row) => {
            // console.log(row)
            return row.disbursement_date !== null ? (
              <Pill color={trxStatusColor["paid"]}>
                {toSentenceCase("disbursed")}
              </Pill>
            ) : (
              <Pill color={trxStatusColor["requested"]}>
      ...
      return [
        { Header: "ID", accessor: "id" },
        {
          Header: "Ref Code",
          accessor: (row) => (
            <Transaction items={[row.ref_code]} trxcode={row.ref_code} />
          ),
        },
        {
          Header: "Assignee Fee",
          accessor: (row) => toMoney(row.assignee_fee),
        },
        {
          Header: "Completed On",
          accessor: (row) => dateTimeFormatterCell(row.completed_on),
        },
        {
          Header: "Disbursement Status",
          accessor: (row) =>
        ...
        },
        {
          Header: "Payment Channel",
          accessor: (row) =>
            toSentenceCase(row.disbursement_bank).toUpperCase(),
        },
      ];
  }, [type]);
      ...
      <Modal
        ...
        onClick={() => {
          ...
          if (type === "merchant")
            dispatch(
              post(
                endpointTransaction + "/admin/disbursement/merchant/create",
                dataDisburse,
                (res) => {
          ...
          if (type === "courier")
            dispatch(
              post(
                endpointTransaction + "/admin/disbursement/courier/create",
                dataDisburse,
                (res) => {
                  setDisburseModal(false);
                  dispatch(refresh());
            ...
            >
              ...
              {view ? null : (
                <MyButton
                  ...
                  onClick={() => {
                    setSelected(
                      disbursement.items.data.filter(
                        (el) => el && !el.disbursement_date
                      )
                    );
              ...
              <Button
                ...
                onClick={() => {
                  dispatch(
                    downloadTransactionDisbursement(type, merchant, courier)
                  );
                }}
              />
            </div>
              ...
              filters={[
                {
                  ...
                  delete: () => {
                    setSettledStart(today);
                    setSettledEnd(today);
                  },
                  value: isRangeToday(settledStart, settledEnd)
                    ? "Today"
