...
const columns = [
  ...
  { Header: "Payment Amount", accessor: (row) => toMoney(row.payment_amount) },
  {
    Header: "Payment Date",
    accessor: (row) =>
      row.payment_date ? dateTimeFormatterCell(row.payment_date) : "Unpaid",
  },
  {
    Header: "Status",
    accessor: (row) =>
...
function Component() {
  ...
  const [trxStart, setTrxStart] = useState(
    moment().startOf("month").format("yyyy-MM-DD")
  );
  const [trxEnd, setTrxEnd] = useState(
      ...
      filters={[
        {
          ...
          value: isRangeToday(trxStart, trxEnd)
            ? "Today"
