...
const columns = [
  {
    ...
    accessor: (row) => (
      <Merchant
        id={row.id}
        data={row}
        items={[<b>{row.name}</b>, <p>{toSentenceCase(row.type)}</p>]}
      />
    ),
  },
  {
    Header: "Category",
    accessor: (row) => (row.category ? toSentenceCase(row.category) : "-"),
  },
  {
    Header: "PIC",
    accessor: (row) => (
    ...
    accessor: (row) => {
      return (
        <Tile
          items={[
            row.is_open === 1 ? (
              <Pill color="primary">Open</Pill>
            ) : (
              <Pill color="secondary">Closed</Pill>
            ),
          ]}
        />
      );
  ...
  },
  {
    Header: "Open Until",
    accessor: (row) =>
      row.is_open === 1 ? dateTimeFormatterCell(row.is_open_until) : "-",
  },
];
    ...
    <>
      <Modal
        ...
        onClick={() => {
          dispatch(
            patch(
              endpointMerchant + "/admin/bulk/setonoff",
              {
                merchant_id: multiActionRows,
                status: merchantStatus,
                is_open_until:
                  merchantStatus === "open"
                    ? inputDateTimeFormatter24(openDate)
                    : null,
              },
              (res) => {
                dispatch(
