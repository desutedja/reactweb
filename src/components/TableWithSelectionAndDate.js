...
function Component({
    ...
    useEffect(() => {
        fetchData && fetchData(pageIndex, pageSize, searchToggle,moment(startDate).format('YYYY-MM-DD'),moment(endDate).format('YYYY-MM-DD'),
            ...sortBy.length > 0 ? [sortField, sortType] : []
        );
    }, [fetchData, pageIndex, pageSize, searchToggle,moment(startDate).format('YYYY-MM-DD'),moment(endDate).format('YYYY-MM-DD'), sortField, sortType]);
